REPLACE INTO CX_RUL_Testdata_v1_LoadCollective OVERWRITE ALL
WITH
  JSON AS (
    SELECT *
    FROM TABLE(
  ...
  LC AS (
    SELECT
      catenaXId,
      JSON_QUERY("urn:bamm:io.openmanufacturing.digitaltwin:1.0.0#ClassifiedLoadCollective",'$[0]') as lc
    FROM JSON
    ),
  FLAT AS (
    SELECT
